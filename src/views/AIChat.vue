...
const sendMessage = async () => {
  ...
  const sourceEvent = new EventSource("http://localhost:8080/ai/chat?chatId="+encodeURIComponent(currentChatId.value)+"&prompt="+encodeURIComponent(messageContent))
  let accumulatedContent = ""
  sourceEvent.onmessage = async function (event) {
    if (event.data === "end") {
      sourceEvent.close()
      return
    }
    accumulatedContent += JSON.parse(event.data).message
    await nextTick(() => {
      // 更新消息，使用累积的内容
  ...
  /*try {
    ...
    while (true) {
      try {
        ...
        // 累积新内容
        let newMessage = decoder.decode(value)
        console.log(newMessage)
        accumulatedContent += newMessage  // 追加新内容
